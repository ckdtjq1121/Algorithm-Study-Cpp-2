...
int32_t main()
{
    ...
    int distance = drazil-dreamoon ;
    double answer ;
    if((distance+random)%2!=0 || random<abs(distance)) //can't reach the destination no matter how
        answer = 0 ;
    else
    {
        int m = (random+abs(distance))/2 ; //moves needed toward the distance m is abs(distance)+(moves-abs(distance))/2
